...
public class RobotContainer {
	...
	private void configureButtonBindings() {
		...
		// // Bind intake eject
		// joystickButton7.whenPressed(() -> m_intake.run(-1));
		// joystickButton7.whenReleased(() -> m_intake.stop());

		// // Bind intake pull
		// joystickButton8.whenPressed(() -> m_intake.run(1));
		// joystickButton8.whenReleased(() -> m_intake.stop());

		// // Bind Lifter Up
		// joystickButton9.whenPressed(() -> m_lifter.run(1));
		// joystickButton9.whenReleased(() -> m_lifter.stop());

		// // Bind Lifter Down
		// joystickButton10.whenPressed(() -> m_lifter.run(-1));
		// joystickButton10.whenReleased(() -> m_lifter.stop());

		// Create function binding
		// joystickButton1.whenPressed(() -> m_intake.run(1, 1));
		// joystickButton1.whenReleased(() -> m_intake.stop());
		// joystickButton1.whenPressed(new RunCommand(() -> m_intake.run(1, 1)));
		// joystickButton1.whenReleased(new RunCommand(() -> m_intake.stop()));
		// joystickButton1.whenHeld(new RunCommand(() -> m_lifter.run(1, 1)));
		// joystickButton1.whileHeld(new RunCommand(() -> m_lifter.run(1, 1)));
		// joystickButton1.whenReleased(new RunCommand(() -> m_lifter.run(0, 1)));
	}
